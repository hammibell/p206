...
try:
    ...
    brake = lambda brake: carla.VehicleControl(hand_brake = True)
    time.sleep(4)

    steer = lambda steer: carla.VehicleControl(steer = steer)
    dropped_vehicle.apply_control(steer(-0.6))

    brake = lambda brake: carla.VehicleControl.apply_control(hand_brake = True)
    time.sleep(6)

    speed = lambda speed: carla.VehicleControl.apply_control()
...
speed = lambda speed: carla.VehicleControl.apply_control(throttle = speed)
dropped_vehicle.apply_control(speed(0.5))

brake = lambda brake: carla.VehicleControl.apply_control(hand_brake = True)
time.sleep(3)

steer = lambda steer: carla.VehicleControl.apply_control(steer = steer)
dropped_vehicle.apply_control(steer(-0.6))
